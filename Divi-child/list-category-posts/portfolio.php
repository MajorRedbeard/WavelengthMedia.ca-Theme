...
$start .= '<ul class="lcp_catlist featured portfolio">';
...
//  echo 'Featured: '.$featured.'<br />URL: '.$url.'<br />Desc: '.$description.'<br />';
  ...
  if ($old) {
	  $old_sites .= "<li class='website'>";
    //Show the title and link to the post:
$old_sites .= '<strong>'.get_the_title().'</strong> ['.$description.']';
    $old_sites .= '</li>';  
	continue;
	  
  } else if ($featured) {
	    //Post Thumbnail
		$featured_sites .= "<li class='website'>";
  $featured_sites .= '<a target="_blank" href="'.$url.'">'.get_the_post_thumbnail().'</a>';
    //Show the title and link to the post:
$featured_sites .= '<a target="_blank" href="'.$url.'"><h2>'.get_the_title().'</h2></a>';
    $featured_sites .= '</li>';
  } /* else { */
  ...
  $lcp_display_output .= "<li class='website'>";

  //Show the title and link to the post:
  //$lcp_display_output .= $this->get_post_title($post, 'h3', 'lcp_post');
$lcp_display_output .= '<a target="_blank" href="'.$url.'" target="_blank"><strong>'.get_the_title().'</strong></a> ['.$description.']';
...
$this->lcp_output = $start.$featured_sites.'</ul>'.$regularheading.'<ul class="regular portfolio">'.$lcp_display_output.'</ul>'.$oldheading.'<ul class="old portfolio">'.$old_sites.$end;